...
import { AudienceModePicker, type VisitMode } from '@/components/audience-experience';
import { ThemeEntrances } from '@/components/theme-entrances';
...
export default function Home() {
  ...
  const [path, setPath] = useState<'waterway' | 'bogong' | null>(null);
  ...
  useEffect(() => {
    ...
    setVisited(stored);
    return () => { if (fadeTimerRef.current !== null) window.clearInterval(fadeTimerRef.current); audioRef.current?.pause(); };
  }, []);
  ...
  function beginJourney(selectedPath?: 'waterway' | 'bogong') { if (selectedPath) { setPath(selectedPath); window.location.href = selectedPath === 'waterway' ? '/themes/waterway-traces' : '/themes/bogong-guards-water'; return; } document.querySelector('#themes')?.scrollIntoView({ behavior: 'smooth' }); }
  function selectVisitMode(mode: VisitMode, target: string) {
    setVisitMode(mode);
    if (target.startsWith('/')) { window.location.href = target; return; }
    window.requestAnimationFrame(() => document.querySelector(`#${target}`)?.scrollIntoView({ behavior: 'smooth' }));
  }
    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section className="relative isolate min-h-screen overflow-hidden">
        ...
        <header className="mx-auto flex w-full max-w-[1480px] items-center justify-between px-5 py-5 sm:px-8 lg:px-12">
          ...
          <div className="flex items-center gap-2"><a href="#themes" className="hidden rounded-full border-2 border-white/70 bg-white/15 px-5 py-2.5 text-sm font-bold text-white backdrop-blur-md transition hover:-translate-y-0.5 hover:bg-white/25 md:block">兩大主題</a><Button onClick={toggleWaterSound} className="sticker-shadow h-11 rounded-full border-2 border-[#113f50] bg-[#ffcb47] px-4 font-black text-[#173c4c] hover:bg-[#ffda78]" aria-pressed={soundOn}>{soundOn ? <Volume2 /> : <VolumeX />}{soundOn ? '水聲 ON' : '水聲 OFF'}</Button></div>
        </header>
        <div id="top" className="mx-auto flex min-h-[calc(100vh-86px)] w-full max-w-[1480px] items-end px-5 pb-10 sm:px-8 sm:pb-14 lg:px-12 lg:pb-16">
          <div className="grid w-full gap-9 lg:grid-cols-[minmax(0,1fr)_430px] lg:items-end">
            <div className="max-w-4xl text-white">
              ...
              <p className="mt-7 max-w-2xl text-base font-medium leading-8 text-white/88 sm:text-lg">先從「圳路築跡」看客庄如何被水利塑造，再從「伯公守水口」理解人們如何面對水的風險。兩條主題路徑，各通往三個獨立展區。</p>
            </div>
            <aside className="sticker-shadow rounded-[30px] border-[3px] border-[#153e4d] bg-[#fff5d5] p-5 text-[#153e4d] sm:p-6 lg:-rotate-1">
              ...
              <div className="mt-5 grid grid-cols-2 gap-3"><button onClick={() => beginJourney('waterway')} className={`path-card rounded-2xl border-2 border-[#153e4d] bg-[#75d4ed] p-4 text-left ${path === 'waterway' ? '-translate-y-0.5 shadow-[0_4px_0_#153e4d]' : ''} hover:bg-[#9de3f3]`}><span className="text-[10px] font-black tracking-[.15em] opacity-55">MATERIAL · 水利</span><span className="mt-7 block text-xl font-black">圳路築跡</span><span className="mt-1 block text-xs font-medium opacity-65">辨水、造水、分水</span></button><button onClick={() => beginJourney('bogong')} className={`path-card rounded-2xl border-2 border-[#153e4d] bg-[#ff8069] p-4 text-left ${path === 'bogong' ? '-translate-y-0.5 shadow-[0_4px_0_#153e4d]' : ''} hover:bg-[#ff9b88]`}><span className="text-[10px] font-black tracking-[.15em] opacity-55">SPIRIT · 守護</span><span className="mt-7 block text-xl font-black">伯公守水口</span><span className="mt-1 block text-xs font-medium opacity-65">守水、敬水、共水</span></button></div>
              <Button onClick={() => beginJourney()} className="mt-4 h-13 w-full rounded-full border-2 border-[#153e4d] bg-[#167f79] text-base font-black text-white hover:bg-[#126d69]">查看兩大主題 <ArrowDown /></Button><p className="mt-3 text-center text-[11px] font-bold opacity-55">2 條策展路徑 · 6 個獨立展區 · 可隨時靜音</p>
            </aside>
          </div>
        </div>
      </section>

      <nav className="sticky top-0 z-40 border-b-2 border-[#143f4f] bg-[#fff9e9]/95 backdrop-blur-xl" aria-label="展覽導覽"><div className="mx-auto flex max-w-[1480px] items-center gap-4 px-5 py-3 sm:px-8 lg:px-12"><a href="#top" className="flex items-center gap-2 font-black tracking-[.08em]"><span className="grid size-8 place-items-center rounded-full bg-[#75d4ed]"><Droplet className="size-4 fill-current" /></span>水水客庄</a><div className="hidden items-center gap-1 lg:flex"><a href="/themes/waterway-traces" className="rounded-full px-3 py-2 text-xs font-black hover:bg-[#dff4f8]">圳路築跡</a><a href="/themes/bogong-guards-water" className="rounded-full px-3 py-2 text-xs font-black hover:bg-[#ffe0d9]">伯公守水口</a><a href="/passport" className="rounded-full px-3 py-2 text-xs font-black hover:bg-[#e6f0bd]">水水護照</a><a href="/about" className="rounded-full px-3 py-2 text-xs font-black hover:bg-[#eee5f8]">關於展覽</a></div><a href="/library" className="ml-auto rounded-full border-2 border-[#143f4f] bg-[#ffcb47] px-4 py-2 text-xs font-black hover:bg-[#ffdc75]">水水書房</a><div className="hidden min-w-32 sm:block sm:w-52"><Progress value={progress} className="gap-1.5 [&_[data-slot=progress-indicator]]:bg-[#1ba29a] [&_[data-slot=progress-track]]:h-2 [&_[data-slot=progress-track]]:border [&_[data-slot=progress-track]]:border-[#153e4d]/20 [&_[data-slot=progress-track]]:bg-[#dce9dc]"><ProgressLabel className="text-[11px] font-black">護照進度 {visited.length}/6</ProgressLabel><ProgressValue className="text-[11px] font-black" /></Progress></div></div></nav>

      <AudienceModePicker selectedMode={visitMode} onSelect={selectVisitMode} />

      <ThemeEntrances />

      <section id="exhibition" className="soft-grid relative px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
        <div className="mx-auto max-w-[1480px]">
          <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr] lg:items-end"><div><Badge className="sticker-mini h-auto rounded-full border-2 border-[#143f4f] bg-[#ffcb47] px-4 py-2 font-black text-[#143f4f]"><Flag /> SIX EXHIBITIONS · 六個獨立展區</Badge><h2 className="mt-5 text-5xl font-black leading-[1.05] tracking-[-.04em] sm:text-7xl">選一扇門，<br /><span className="text-[#148781]">走進一條水路。</span></h2></div><p className="max-w-3xl text-lg font-medium leading-9 text-[#4e6974]">每個展區都是獨立網頁，包含主題影音、直式 Shorts、客語繪本、專屬遊戲與 O2O 現地任務。完成遊戲後，線索會跨頁保留。</p></div>
          <div className="relative mt-12 rounded-[26px] border-2 border-[#143f4f] bg-white p-4 sticker-mini sm:p-6"><div className="absolute left-[8%] right-[8%] top-1/2 hidden border-t-2 border-dashed border-[#143f4f]/30 md:block" /><div className="relative grid grid-cols-3 gap-3 md:grid-cols-6">{zones.map((zone) => { const collected = visited.includes(zone.id); return <a href={`/zones/${zone.slug}`} key={zone.id} className="group flex flex-col items-center gap-2 text-center"><span style={{ backgroundColor: collected ? zone.color : '#fff9e9' }} className="grid size-12 place-items-center rounded-full border-2 border-[#143f4f] text-sm font-black transition group-hover:-translate-y-1 sm:size-14">{collected ? <Check className="size-5" /> : zone.index}</span><span className="text-[11px] font-black text-[#58707a]">{collected ? zone.clue : `進入 ${zone.index}`}</span></a>; })}</div></div>
          <div className="mt-12 grid gap-7 md:grid-cols-2 xl:grid-cols-3">
            {zones.map((zone, index) => { const collected = visited.includes(zone.id); return (
              <article id={zone.id} key={zone.id} className={`mission-card scroll-mt-28 overflow-hidden rounded-[30px] border-[3px] border-[#143f4f] bg-white ${index % 3 === 1 ? 'xl:translate-y-6' : ''}`}>
                ...
                <div className="flex min-h-[360px] flex-col p-5 pt-3 sm:p-6 sm:pt-3"><div className="flex flex-wrap gap-2"><Badge style={{ backgroundColor: zone.color, color: zone.ink }} className="h-auto border-2 border-[#143f4f] px-3 py-1 font-black">{zone.verb}</Badge><Badge variant="outline" className="h-auto border-2 border-[#143f4f]/25 px-3 py-1 font-bold text-[#38727a]">獨立展區網頁</Badge></div><h3 className="mt-5 text-3xl font-black leading-tight tracking-[-.02em]">{zone.title}</h3><p className="mt-3 font-black text-[#e65e4b]">{zone.subtitle}</p><p className="mt-4 text-sm font-medium leading-7 text-[#58707a]">{zone.description}</p><div className="mt-auto pt-6"><div className="border-t-2 border-dashed border-[#143f4f]/20 pt-5"><p className="text-[10px] font-black tracking-[.14em] text-[#7a9097]">影音 · 繪本 · 遊戲 · O2O</p><p className="mt-1 text-lg font-black">{zone.interaction}</p><a href={`/zones/${zone.slug}`} className={`mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-full border-2 border-[#143f4f] font-black text-[#143f4f] ${collected ? 'bg-[#bddd43]' : 'bg-[#75d4ed]'} hover:bg-[#ffcb47]`}>{collected ? <Check /> : <Play className="fill-current" />}{collected ? `再次進入：${zone.clue}` : '進入完整展區'}</a><div className="mt-4 flex justify-end"><a href={zone.source} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs font-bold text-[#748a92] hover:text-[#143f4f]" aria-label="查看典藏來源">典藏來源 <ExternalLink className="size-4" /></a></div></div></div></div>
              </article>
            ); })}
          </div>
        </div>
      ...
      <footer className="bg-[#0b3646] px-5 py-12 pb-28 text-white sm:px-8 lg:px-12 md:pb-12"><div className="mx-auto flex max-w-[1480px] flex-col gap-8 border-b border-white/15 pb-10 md:flex-row md:items-end md:justify-between"><div><p className="text-3xl font-black tracking-[.1em]">水水客庄</p><p className="mt-3 max-w-xl text-sm font-medium leading-7 text-white/60">從一滴水看見客庄，從客庄智慧認同自己；與自然共生，與地方共榮。</p></div><div className="flex flex-wrap gap-5 text-sm font-bold text-white/70"><a href="#themes" className="hover:text-[#bddd43]">兩大主題</a><a href="#exhibition" className="hover:text-[#bddd43]">六個獨立展區</a><a href="/library" className="hover:text-[#bddd43]">水水書房</a><a href="/passport" className="hover:text-[#bddd43]">水水護照</a><a href="/about" className="hover:text-[#bddd43]">關於展覽</a><button onClick={() => setAssistantOpen(true)} className="hover:text-[#bddd43]">AI 小幫手</button></div></div><div className="mx-auto flex max-w-[1480px] flex-col gap-2 pt-6 text-xs text-white/40 sm:flex-row sm:justify-between"><p>策展原型內容依《水水客庄線上展示計畫服務建議書修訂版 v0.6》製作</p><p>展示圖像來源：客家文化發展中心客家文化資產數位網</p></div></footer>
