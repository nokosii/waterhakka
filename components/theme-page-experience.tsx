...
export function ThemePageExperience({ theme }: { theme: ThemeRecord }) {
  ...
  return (
    <>
      ...
      <section className="border-t-[3px] border-[#143f4f] bg-[#ffcb47] px-5 py-12 sm:px-8 lg:px-12"><div className="mx-auto grid max-w-[1480px] gap-5 md:grid-cols-3"><ActionLink href={`/passport?theme=${theme.slug}`} icon={MapPinned} title={theme.o2oTitle} note={theme.o2oNote} /><ActionLink href={`/library?theme=${theme.slug}`} icon={Bot} title="客語 AI 導覽" note="前往水水書房，查詢客語詞彙與文化延伸資料。" /><ActionLink href={`/library?theme=${theme.slug}`} icon={Sparkles} title="主題延伸資料" note={`帶入「${theme.tags.slice(0, 4).join('、')}」等主題標籤。`} /></div></section>
    </>
  );
}
